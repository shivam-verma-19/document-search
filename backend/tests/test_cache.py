...
def _get_cache():
    """Reload real cache module so its module-level DynamoDB table is re-bound."""
    import backend.app.cache as m
...
# ---------------------------------------------------------------------------
# hash_query — pure function, no AWS needed
# ---------------------------------------------------------------------------
...
class TestCacheOperations:
    ...
    def test_cache_miss_returns_none(self):
        _make_table()
        m = _get_cache()
        assert m.get_cache("nonexistent query") is None

    @mock_aws
    def test_set_then_get_returns_answer(self):
        _make_table()
        m = _get_cache()
        m.set_cache("what is AI?", "AI is artificial intelligence.")
        assert m.get_cache("what is AI?") == "AI is artificial intelligence."

    @mock_aws
    def test_overwrite_returns_latest(self):
        _make_table()
        m = _get_cache()
        m.set_cache("q", "first answer")
        m.set_cache("q", "second answer")
        assert m.get_cache("q") == "second answer"

    @mock_aws
    def test_different_queries_independent(self):
        _make_table()
        m = _get_cache()
        m.set_cache("query A", "answer A")
        m.set_cache("query B", "answer B")
        assert m.get_cache("query A") == "answer A"
        assert m.get_cache("query B") == "answer B"
    ...
    def test_cache_stores_long_answer(self):
        ...
        m.set_cache("long q", long_answer)
        assert m.get_cache("long q") == long_answer
    ...
    def test_whitespace_sensitive(self):
        ...
        m = _get_cache()
        m.set_cache("hello", "a")
        assert m.get_cache(" hello") is None
